...
const AdminDashboardPage = () => {
  const { user, logout, isAuthenticated, isConnectionUnlimitedUser } = useAuth()
  ...
  if (!isAuthenticated || !isConnectionUnlimitedUser) {
      ...
      >
        ...
        <div className="space-y-3">
          {[
            ...
            { user: 'admin@connectionunlimited.com', action: 'generated QR code', time: '10 min ago', type: 'qr' },
            { user: 'mike@startup.com', action: 'updated profile', time: '15 min ago', type: 'profile' }
          ].map((activity, index) => (
            <div key={index} className="flex items-center space-x-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
              <div className={`w-2 h-2 rounded-full ${activity.type === 'order' ? 'bg-red-500' :
                  ...
                  <nav className="space-y-2">
                    {navigation.map((item) => (
                      ...
                      >
                        ...
                        {item.badge && (
                          <span className={`px-2 py-1 text-xs rounded-full ${item.urgent ? 'bg-red-500 text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300'
                            }`}>
                            {item.badge}
                          </span>
                        )}
